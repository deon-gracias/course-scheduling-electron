...
export default function CoursesForm({ setSchedule }) {
  ...
  function updateCourse(index, name, enrollment) {
    if (name !== null) {
  ...
  function submitHandler() {
    ...
    try {
      const scheduledData = schedule(
        convertArrayToObject(courses),
        convertArrayToObject(rooms),
        times,
        preferences
      );
          ...
          <React.Fragment key={`course-${index}`}>
            ...
            <Grid.Col span={4}>
              <NumberInput
                ...
                onChange={(e) => updateCourse(index, null, e.target.value)}
