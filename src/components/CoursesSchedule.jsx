      ...
      {schedule !== "" && (
        ...
        >
          ...
          <Tabs defaultValue={"Standard"}>
            <Tabs.List>
              {views.map((e) => (
                <Tabs.Tab key={`tab-${e.name}`} value={e.name}>
                  {e.name}
                </Tabs.Tab>
              ))}
            </Tabs.List>
            ...
            {typeof schedule === "object" ? (
              views.map((e) => (
                <Tabs.Panel key={`tab-panel-${e.name}`} value={e.name}>
                  {e.component}
                </Tabs.Panel>
              ))
            ) : (
...
function RoomView({ schedule }) {
  return Object.entries(schedule.roomWise).map(([room, course]) => (
    <Box sx={{ padding: "1rem" }}>
...
function CourseView({ schedule }) {
  return (
    schedule.courseWise !== undefined &&
    Object.entries(schedule.courseWise).map(([course, room]) => (
      <Box sx={{ padding: "1rem" }}>
        <Title ml={"0.5rem"} order={4}>
          Course {course}
        </Title>
